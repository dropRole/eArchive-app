...
(() => {
    ...
    function addGraduation(e, pNode) {
        ...
        if (e.target.checked) {
            ...
            fLbl.setAttribute('for', `fInpt${i}`)
            dLbl.textContent = 'Zagovorjen'
            dLbl.setAttribute('for', `dInpt${i}`)
            iLbl.textContent = 'Izdan'
            iLbl.setAttribute('for', `iInpt${i}`)
            fInpt.id = `fInpt${i}`
            fInpt.type = 'file'
            fInpt.required = true
            dInpt.id = `dInpt${i}`
            ...
            dInpt.type = 'date'
            dInpt.required = true
            dInpt.name = `attendances[${i}][defended]`
            iInpt.id = `iInpt${i}`
            iInpt.className = 'form-control'
            iInpt.type = 'date'
            iInpt.name = `attendances[issued][${i}]`
            iInpt.required = true
    ...
    function addAttendance() {
        ...
        p.textContent = `${i}. študijski program`
        ...
        fLbl.setAttribute('for', `fSlct${i}`)
        fLbl.textContent = 'Fakulteta'
        pLbl.textContent = 'Program'
        pLbl.setAttribute('for', `pSlct${i}`)
        gLbl.textContent = 'Diplomiral'
        gLbl.setAttribute('for', `gCb${i}`)
        gLbl.className = 'mt-2'
        fSlct.className = 'form-control'
        fSlct.id = `fSlct${i}`
        fSlct.name = `attendances[facluties][${i}]`
        fSlct.required = true
        pSlct.className = 'form-control'
        pSlct.id = `pSlct${i}`
        pSlct.name = `attendances[programs][${i}}]`
        pSlct.required = true
        gCb1.id = `gCb${i}`
        gCb1.type = 'checkbox'
        gCb1.classList = 'mr-2'
