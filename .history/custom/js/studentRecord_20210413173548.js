...
(() => {
    ...
    function addGraduation(e, pNode) {
        // if it's not checked
        if (e.target.checked) {
            // get graduation label form control counter
            let cntr = e.target.getAttribute('data-counter'),
                // get attendances array current row index
                indx = e.target.getAttribute('data-index') == 0 ? '' : e.target.getAttribute('data-index'),
                // create form controls 
                fGDiv = document.createElement('div'),
                fGDiv1 = document.createElement('div'),
                fGDiv2 = document.createElement('div'),
            ...
            fLbl.setAttribute('for', `fInpt${indx}`)
            dLbl.textContent = 'Zagovorjen'
            dLbl.setAttribute('for', `dInpt${indx}`)
            iLbl.textContent = 'Izdan'
            iLbl.setAttribute('for', `iInpt${indx}`)
            fInpt.id = `fInpt${indx}`
            fInpt.type = 'file'
    ...
    function addAttendance() {
        ...
        p.textContent = `${aLblCntr}. študijski program`
        ...
        gCb.setAttribute('data-coutner', aLblCntr)
