...
(() => {
    ...
    function addSojourn() {
        ...
        aLbl.setAttribute('for', `aInpt${i + 1}`)
        aLbl.textContent = `${i + 1}. naslov`
        pCLbl.setAttribute('for', `pCSlct${i + 1}`)
        pCLbl.textContent = 'Kraj'
        cLbl.setAttribute('for', `cSlct${i + 1}`)
    ...
    function addGraduation(e, pNode) {
        ...
        if (e.target.checked) {
            let index = 0
                // if derived value is a number
            if (!isNaN(e.target.id.substring(e.target.id.length, e.target.id.length - 1)))
            // derive attendance array index number 
                index = e.target.id.substring(e.target.id.length, e.target.id.length - 1)
                // create form controls 
            ...
            fLbl.setAttribute('for', `fInpt${index == 0 ? '' : index}`)
            dLbl.textContent = 'Zagovorjen'
            dLbl.setAttribute('for', `dInpt${index == 0 ? '' : index}`)
            iLbl.textContent = 'Izdan'
            iLbl.setAttribute('for', `iInpt${index == 0 ? '' : index}`)
            fInpt.id = `fInpt${index == 0 ? '' : index}`
            fInpt.type = 'file'
            ...
            cInpt.type = 'hidden'
            cInpt.name = `attendances[${index == 0 ? '' : index}][certificate]`
            dInpt.id = `dInpt${index == 0 ? '' : index}`
            dInpt.className = 'form-control'
            dInpt.type = 'date'
            dInpt.required = true
            dInpt.name = `attendances[${index == 0 ? '' : index}][defended]`
            iInpt.id = `iInpt${index == 0 ? '' : index}`
            iInpt.className = 'form-control'
            iInpt.type = 'date'
            iInpt.name = `attendances[${index == 0 ? '' : index}][issued]`
            iInpt.required = true
    ...
    function addAttendance() {
        ...
        p.textContent = `${j + 1}. študijski program`
        ...
        fLbl.setAttribute('for', `fSlct${j + 1}`)
        fLbl.textContent = 'Fakulteta'
        pLbl.textContent = 'Program'
        pLbl.setAttribute('for', `pSlct${j + 1}`)
        gLbl.textContent = 'Diplomiral'
        gLbl.setAttribute('for', `gCb${j + 1}`)
        gLbl.className = 'mt-2'
        fSlct.className = 'form-control'
        fSlct.id = `fSlct${j + 1}`
        fSlct.name = `attendances[${j}][id_faculties]`
        fSlct.required = true
        pSlct.className = 'form-control'
        pSlct.id = `pSlct${j + 1}`
        pSlct.name = `attendances[${j}][id_programs]`
        pSlct.required = true
        gCb.id = `gCb${j + 1}`
        gCb.type = 'checkbox'
        gCb.classList = 'mr-2'
        gCb.setAttribute('data-index', j - 1)
