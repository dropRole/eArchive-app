...
(() => {
    ...
    function addSojourn() {
        ...
        aInpt.type = 'text'
        aInpt.name = `sojourns[${i - 1}][address]`
        aInpt.required = true
        pCSlct.id = `pCSlct${i}`
        pCSlct.classList = 'form-control'
        pCSlct.name = `sojourns[${i - 1}][id_postal_codes]`
    ...
    function addGraduation(e, pNode) {
        ...
        if (e.target.checked) {
            ...
            dInpt.type = 'date'
            dInpt.required = true
            dInpt.name = `attendances[${j + 1}][defended]`
            iInpt.id = `iInpt${j}`
            iInpt.className = 'form-control'
            iInpt.type = 'date'
            iInpt.name = `attendances[${j + 1}][issued]`
            iInpt.required = true
    ...
    function addAttendance() {
        ...
        fSlct.id = `fSlct${j}`
        fSlct.name = `attendances[${j - 1}][faculties]`
        fSlct.required = true
        pSlct.className = 'form-control'
        pSlct.id = `pSlct${j}`
        pSlct.name = `attendances[${j - 1}][programs]`
        pSlct.required = true
        gCb1.id = `gCb${j}`
        gCb1.type = 'checkbox'
        gCb1.classList = 'mr-2'
