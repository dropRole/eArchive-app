...
(() => {
    ...
    function addGraduation(e, pNode) {
        ...
        if (e.target.checked) {
            ...
            fInpt.addEventListener('change', e => {
                    ...
                    console.log(cInpt.value)
                    cInpt.name = `attendances[${index}][certificate]`
                }) // addEventListener
            dInpt.id = `dInpt${index == 0 ? '' : index}`
            dInpt.className = 'form-control'
            dInpt.type = 'date'
            dInpt.required = true
            dInpt.name = `attendances[${index}][defended]`
            iInpt.id = `iInpt${index == 0 ? '' : index}`
            iInpt.className = 'form-control'
            iInpt.type = 'date'
            iInpt.name = `attendances[${index}][issued]`
            iInpt.required = true
            cInpt.type = 'hidden'
    ...
    function addAttendance() {
        ...
        fSlct.id = `fSlct${j + 1}`
        fSlct.name = `attendances[${j}][faculties]`
        fSlct.required = true
        pSlct.className = 'form-control'
        pSlct.id = `pSlct${j + 1}`
        pSlct.name = `attendances[${j}][programs]`
        pSlct.required = true
        gCb1.id = `gCb${j + 1}`
        gCb1.type = 'checkbox'
        gCb1.classList = 'mr-2'
