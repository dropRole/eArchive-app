...
class DBC extends PDO
{
    ...
    */
    public function insertStudent(int $id_postal_codes, string $name, string $surname, string $email = NULL, string $telephone = NULL, $residences = [])
    {
            ...
            foreach ($residences as $residence) {
                ...
                try {
                    ...
                    if ($prpStmt->rowCount() == 1) {
                        ...
                        if ($permanent)
                            $report['message'] .= 'Stalno prebivališče je uspešno dodano.' . PHP_EOL;
                        // if residence is temporal
                        if (!$permanent) {
                            $report['message'] .= "{$i}. začasno bivališče je uspešno dodano." . PHP_EOL;
                            $i++;
                        } // if
                    } //if
                } // try
        ...
        $report['message'] = 'Napaka: osnovni podakti študenta ter podatki o prebivališču niso uspešno vstavljeni.'. var_dump($prpStmt->errorInfo());
        return $report;
    } // insertStudent
