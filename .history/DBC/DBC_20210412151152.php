    ...
    */
    public function insertStudent(int $id_postal_codes, string $name, string $surname, string $email = NULL, string $telephone = NULL, $sojourns = [])
    {
        ...
        if ($prpStmt->rowCount() == 1) {
            ...
            $i = 1; // counter
            for ($i = 0; $i < count($sojourns['id_postal_codes']); $i++) {
                $stmt = '   INSERT INTO 
                                sojourns
                            (
                                id_postal_codes,
                ...
                try {
                    // prepare, bind params to and execute stmt
                    $prpStmt = $this->prepare($stmt);
                    $prpStmt->bindParam(':id_postal_codes', $sojourns['id_postal_codes'][$i], PDO::PARAM_INT);
                    $prpStmt->bindParam(':id_students', $id_students, PDO::PARAM_INT);
                    $prpStmt->bindParam(':address', $sojourns['address'][$i], PDO::PARAM_STR);
                    $prpStmt->execute();
                } // try
                catch (PDOException $e) {
                    echo "Napaka: {$e->getMessage()}.";
