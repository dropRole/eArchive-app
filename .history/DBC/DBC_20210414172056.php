...
class DBC extends PDO
{
    ...
    */
    public function insertStudent(int $id_postal_codes, string $name, string $surname, string $email = NULL, string $telephone = NULL, $residences = [])
    {
        ...
        $report['message'] = 'Napaka: osnovni podakti študenta ter podatki o bivališču niso uspešno vstavljeni.';
        return $report;
    } // insertStudent
    ...
    */
    public function insertAttendances(int $id_students, int $id_faculties, int $id_programs, DateTime $enrolled, int $index)
    {
        ...
        if ($prpStmt->rowCount() == 1) {
            ...
            $student = selectStudentsByIndex($index)[0]; 
            $report['message'] = "Podatki o študijskem programu {$student->program} so uspešno vstavljeni.";
            return $report;
        } // if
        $report['message']
