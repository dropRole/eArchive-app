...
class DBC extends PDO
{
        ...
        if ($prpStmt->rowCount() == 1) {
            ...
            foreach ($residences as $residence) {
                ...
                try {
                    ...
                    $prpStmt->bindParam(':status', $permanent ? 'STALNO' : 'ZAČASNO', PDO::PARAM_STR);
                    $prpStmt->execute();
                    // if single row is affected
                    if ($prpStmt->rowCount() == 1) {
                        // if residence is permament
                        if ($permanent)
                            $report['message'] .= 'Stalno prebivališče je uspešno dodano.';
                        // if residence is temporal
                        if (!$permanent) {
                            $report['message'] .= "{$i}. začasno bivališče je uspešno dodano.";
                            $i++;
                        } // if
                    } //if
                } // try
    ...
    */
    public function insertGraduation(int $id_attendances, DateTime $defended, string $lookup)
    {
        // report
        $report = '';
        ...
        if ($this->inTransaction()) {
            try {
                ...
                foreach ($_FILES['certificate']['tmp_name'] as $indx => $tmp_name) {
                    // if certificate name matches the one to lookup for
                    if ($_FILES['certificate']['name'][$indx] == $lookup) {
                        // if certificate is uploaded successfully 
                        if ($_FILES['certificate']['error'][$indx] == UPLOAD_ERR_OK) {
                            $finfo = new finfo();
                            $mimetype = $finfo->file($tmp_name, FILEINFO_EXTENSION);
                            ...
                            if ($upload) {
                                ...
                                VALUES(
                                    ...
                                    // if single row is affected 
                                    if ($prpStmt->rowCount() == 1) {
                                        $report .= 'Podatki o zaključku študiranja ter certifika so uspešno vstavljeni.' . PHP_EOL;
                                        // commit transaction
                                        $this->commit();
                                        return $report;
                                    } // if
